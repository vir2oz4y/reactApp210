...
const EditProductPopup = ({ open, onClose, Product: ProductEdit, onEdit }: Props) => {
    ...
    const onEditClick = () => {
        ...
        shchegolevaAxios.patch<{ item: Product }>('https://canstudy.ru/orderapi/Product',
            {
                item: {
                    ...Product
                }
    ...
    return (
        <ShchegolevaPopup title={'Edit client'} open={open} onClose={() => onClose()}>
            <div style={{
                    display: 'flex',
                ...
                <TextField
                    label="Name"  variant="standard" fullWidth={true} value={Product.name}
                    onChange={e => setProduct(prev => ({ ...prev, name: e.target.value }))}/>

                <TextField label="Description" variant="standard" fullWidth={true} value={Product.description}
